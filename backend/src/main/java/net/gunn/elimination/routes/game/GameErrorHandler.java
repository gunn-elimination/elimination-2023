...
public class GameErrorHandler {
	...
	public void handleAccessDeniedException(HttpServletResponse response) throws IOException {
		...
		else if (!SecurityContextHolder.getContext().getAuthentication().getAuthorities().contains(USER)
			&& eliminationManager.gameIsOngoing()) {
			response.sendRedirect("/oauth2/authorization/google");
		} else if (!SecurityContextHolder.getContext().getAuthentication().getAuthorities().contains(PLAYER))
